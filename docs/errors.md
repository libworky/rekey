...
| `OAUTH_ID_TOKEN_INVALID` | 502 | The OIDC issuer returned an ID token whose claims do not bind it to this exchange: no `sub`, an `iss` that is not the configured issuer, an `aud` without this client id, or an `exp` in the past. Refused rather than used, because an empty subject would fold every user of the provider onto one account. | Check the issuer URL and client id configured for the provider against what the identity provider issues. |
