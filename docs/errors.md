...
| `USER_TOKEN_INVALID` (revoked) | 401 | The access token predates the user's last password change or reset, sign-out everywhere, session revoke or device release; the message says so. Same code as an unverifiable token, so existing SDKs refresh on it. | Refresh; if the refresh token is gone too, sign in again. |
...
| `BILLING_PROVIDER_INBOUND_ONLY` | 400 | An outbound call (checkout, plan registration) reached the external billing provider, which only receives events. The router never picks it, so this means a caller named it explicitly or a row bound to it was asked to do something only a hosted provider can. | Do it in your billing system, which then posts the resulting event. For self-serve checkout, connect a hosted provider. |
